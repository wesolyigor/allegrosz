...
class BelongsToOtherFieldOption:
    def __init__(self, table, belongs_to, foreign_key=None, message=None):
        if not table:
            ...
            BelongsToOtherFieldOption validator needs the table parameters                 
            ''')
        if not belongs_to:
            raise AttributeError('''
            BelongsToOtherFieldOption validator needs the belongs_to parameters                 
            ''')
        self.table = table
        ...
        if not message:
            message = 'Chosen option is not a valid'
    ...
    def __call__(self, form, field):
        ...
        try:
            ...
            raise AttributeError(f'Passed parameters as not correct.{e}')
